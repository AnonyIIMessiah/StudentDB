...
var connectionToken = "90931304|-31949328562354085|90950458";
...
function validateFormData() {
  ...
  if (!validateEnrollmentDate()) {
    alertHandler(
      0,
      "Invalid Enrollment Date(i.e Enrollment Date should be greater than Birth Date)"
    );
...
function getStudentData() {
  if ($("#rollNo").val() === "") {
    // if roll number is not given then disable all feild
    disableAllFeildExceptRollno();
  } else if ($("#rollNo").val() < 1) {
    // if roll number is not valid (i.e roll-no <1)
    disableAllFeildExceptRollno();
    alertHandler(0, "Invalid Roll-No");
    $("#rollNo").focus();
  } else {
    ...
    if (resJsonObj.status === 400) {
      // if student is not exist already with same roll number then enable save and reset btn
      ...
      // if student is exist already with same roll number then enable update and reset btn
